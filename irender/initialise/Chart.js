...
function IChart() {
	...
	this.setColourPallet();
	this.setOptions();
}
IChart.prototype=Object.create(Svg.prototype);
IChart.prototype.setOptions=function Chart (options) {
...
*/
	...
	if(this.table===null) throw Error("Cannot render chart as table name not specified");
}
IChart.prototype.buildLocalDataSet=function() {
	this.localDataSet=[];
	if(this.historyInclude && this.baseTableData.history!=null) {
		this.baseTableData.history.data.forEach(c=>this.copyFilteredRows(c,this.localDataSet,this.baseTableData.filters,this.chartFilter));
	}
...
IChart.prototype.drawChart_bar=function () {
	...
	for(let d,j,i= 0; i<this.data.length; i++) {
		for(j=this.columnIndexDetails.y.start; j<this.columnIndexDetails.y.end; j++) {
			...
			this.drawObject({action:"rect",
				...
				stroke:(this.outline==="none"?this.colours[j]:this.outline),
...
IChart.prototype.drawChart_bubble=function() {
	let zDetails=this.columnIndexDetails.z,
	...
	for(let data,zData,radius,xPos,yPos,y,i=0; i<this.data.length; i++) {
		for(let j=this.columnIndexDetails.y.start; j<this.columnIndexDetails.y.end; j++) {
			...
			this.drawObject({action:"circle",cx:xPos,cy:xPos,r:radius,stroke:(this.outline==="none"?this.colours[j]:this.outline),"stroke-width":1,fill:colour,"fill-opacity":0.5});
		}
	}
};
...
IChart.prototype.drawChart_pie=function() {
	...
	let xPos=0, yPos=0,
		piesCount=(this.slices=='row'?this.columnIndexDetails.y.size+1:this.data.length),
	...
	for(let i=(this.slices=='row'?0:0); i<piesCount; i++) {
...
IChart.prototype.drawChart_stack=function() {
	...
	for(let xPos=0,yPos=0,y=0,total,d,j,i=0; i<this.data.length; i++) {
		...
		for(j=yStart; j<yEnd; j++) {
			...
			if(d==null) continue;
			y=this.scaleY(d);
			this.drawObject({action:"rect",x:xPos,y:yPos,width:this.barWidth,height:y,stroke:this.outline=="none"?this.colours[j]:this.outline,"stroke-width":this.lineWidth,fill:this.colours[j]});
			yPos+=y;
		}
	}
};
...
IChart.prototype.setColourPallet=function() {
	this.colourPallet=[];
	const delta=255/5;
	let x,j,k;
	for(x=0,i=0;i<5;i++)
...
IChart.prototype.setParameter=function(element) {
	...
	switch (element.name) {
		...
		case 'display':
			...
			element.value== (element.value='chart'?'data':'chart');
	}
};
...
IChart.prototype.renderTableDataBuild=function() {
	if(this.chart.type==='pushline') {
		...
		for(let d,xPos,yPos,i= 0; i<this.data.length; i++) {
			...
			points+= " "+xPos+","+yPos;
			if(this.showPoints)	
				this.drawObject({action:"circle",cx:plotX,cy:plotY,r:3,stroke:colour,"stroke-width":this.lineWidth,fill:colour});
		}
...
IChart.prototype.redrawChart=function() {
	let errors;
	const colourCnt=this.chart.type=='pie'?
	...
	if(colourCnt>this.colour.length) {
		const delta=this.colourPallet.length/colourCnt;
		for(let i=this.colour.length; i<colourCnt; i++) 
			this.colours[i]=this.colourPallet[Math.floor(i*delta)];
	}
	this.clearAll();
	this.drawLineAxis();
	...
	if(this.chart.type=='pie') {
		if(this.slices=='row') {
			for(let i=1; i<this.data.length; i++)
				this.legendNewRow().innerHTML= "<a style='color:" + this.colours[i] + "' >" + this.data[i][0] + "</a>";
		} else 
			for(let i=y.start; i<y.end; i++)
				this.legendNewRow().innerHTML="<a style='color:" + this.colours[i] + "' >" + this.label[i] + "</a>";
	} else  {
		this.legendNewRow().innerHTML="x axis: " + this.label[0];
		for(let i=y.start; i<y.end; i++) {
			this.legendNewRow().innerHTML=
				"<a style='color:" + this.colours[i] + "' >" + this.label[i] +
...
IChart.prototype.drawPie=function(x,y,d,i) {
	...
	for(let row=0; row<data.length; row++) {
		...
		this.drawObject({action:"path",d:describeArc(xCentre,yCentre,radius,angleStart,angleEnd),stroke:colour,"stroke-width":this.lineWidth,fill:colour});
	}
  	if(this.slices=='row')
...
IChart.prototype.plotSet=function(y, offset, data) {
	...
	this.drawObject({action:"polyline",id:"polyline",points:points,stroke:this.colours[y * offset],"stroke-width":this.lineWidth,fill:"none"});
};
IChart.prototype.plot=function(y, offset, data,transparency=1) { 
	if(data.length==0) return;
	let dataY,dataX,plotX,plotY,last,colour=this.colours[y * offset];
	const baseAttributes={stroke:colour,"stroke-width":this.lineWidth,fill:colour,"fill-opacity":transparency};
	...
	for(row= 0; row<data.length; row++) {
		try {
			...
			if(dataX==null || dataY==null || isNaN(dataX) || isNaN(dataY)) {
				if(linePointCnt>0) {
					this.drawObjectAssign(baseAttributes,{action:"polyline",points:points,stroke:colour});
	...
	if(linePointCnt>0) {
		this.drawObjectAssign(baseAttributes,{action:"polyline",id:"polyline",points:points,stroke:colour});
...
IChart.prototype.setData=function(x) {
	this.data[x]=[];
	this.deltaData[x]=[];
};
